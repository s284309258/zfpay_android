...
public class UpdateLoginPwActivity extends BaseActvity {
    ...
    protected void initView() {
        customTitle.setText(getString(R.string.修改登录密码));
    ...
    private void sendVcode() {
        ...
        mMyService.sendSmsCodeToken(new SendSmsCodeTokenRequest(dataManager.getToken(), "FrontModifyLoginPass", mImg_id, ivVcode))
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new BaseConsumer<HttpResponse>(this) {
                    ...
                    public void onSuccessCall(HttpResponse response) {
                        ...
                        CountDownTimerUtils mCountDownTimerUtils = new CountDownTimerUtils(UpdateLoginPwActivity.this, btnSendVcode, 60000, 1000); //倒计时1分钟
                        mCountDownTimerUtils.start();
                        Utils.showMsg(UpdateLoginPwActivity.this, getString(R.string.验证码发送成功));
                    }
    ...
    private void update() {
        ...
        mMyService.modifyLoginPass(new ModifyLoginPassRequest(dataManager.getToken(), pw, vcode))
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new BaseConsumer<HttpResponse>(this) {
                    @Override
                    public void onSuccessCall(HttpResponse response) {
                        ProgressDlgHelper.closeDialog();
                        finish();
                        Utils.showMsg(UpdateLoginPwActivity.this, getString(R.string.修改成功));
                    }
    ...
    private boolean checkEdit() {
        /*if (TextUtils.isEmpty(tvPhone.getText().toString().trim())) {
            ...
            Utils.showMsg(this, getString(R.string.请输入登录密码));
            return false;
